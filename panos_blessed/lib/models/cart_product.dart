import 'package:cloud_firestore/cloud_firestore.dart';
import 'package:flutter/cupertino.dart';
import 'package:panos_blessed/models/item_color.dart';
import 'package:panos_blessed/models/product.dart';

class CartProduct extends ChangeNotifier {
  CartProduct.fromProduct(this.product) {
    productId = product.id;
    quantity = 1;
    color = product.selectedColor.name;
  }

  CartProduct.fromDocument(DocumentSnapshot document) {
    id = document.documentID;
    productId = document.data['pid'] as String;
    quantity = document.data['quantity'] as int;
    color = document.data['color'] as String;

    firestore.document('products/$productId').get().then((doc) {
      product = Product.fromDocument(doc);
      notifyListeners();
    });
  }

  final Firestore firestore = Firestore.instance;

  String id;

  String productId;
  int quantity;
  String color;

  Product product;

  ItemColor get itemColor {
    if (product == null) return null;
    return product.findColor(color);
  }

  num get unitPrice {
    if (product == null) return 0;
    return itemColor?.price ?? 0;
  }

  num get totalPrice => unitPrice * quantity;

  Map<String, dynamic> toCartItemMap() {
    return {
      'pid': productId,
      'quantity': quantity,
      'color': color,
    };
  }

  bool stackable(Product product) {
    return product.id == productId && product.selectedColor.name == color;
  }

  void increment() {
    quantity++;
    notifyListeners();
  }

  void decrement() {
    quantity--;
    notifyListeners();
  }

  bool get hasStock {
    final color = itemColor;
    if (color == null) return false;
    return color.stock >= quantity;
  }
}
